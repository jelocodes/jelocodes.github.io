...
We'd also need to write all of the smart contract functions within the contract's curly braces. We won't go through the entirety of the contract, but, along with some setters, getters and refund logic, the major function that I needed to create was payoutToMaker(), which sends the next increment of a backer's pledge to the maker. This should be envoked by backers every time a maker provides proof that progress has been made on their project. We'll see later that we can envoke this functionality using a button in our UI with some jQuery event triggers.
...
It's beyond the scope of this article to explain the Remix IDE, but know that with it, you can compile your contract directly in the browser and see any errors (click the Compile tab), as well as test any contract functions (click the Run tab). When everything is working, you can deploy the contract onto the blockchain directly from Remix.
...
To set up our app functionality, we use a few different EthJS modules, instantiating them into constants ('Eth' and 'EthContract') for reference throughout the app. We pass in the global Web3 object (injected by MetaMask) upon app start, getting the current provider (the blockchain that Web3 is currently talking to, which can be set and edited in the settings) and initializing it, and all of its functionality, into a variable for reference in our code (which we've called 'eth').
...
Afterwards, we need to set up an instance of EthContract with all of the functionality of our specific compiled contract. By passing in the compiled contract's abi as an argument (which we got from Truffle compilation), and instantiating it in a JS constant (in our case, named 'ThingFunder'), we now have access to all of the contract's functions in JS representation, which we can call in our app and pass to Metamask and the blockchain via the injected Web3.js object's RPC functionality. 
...
Now we're able to trigger contract functionality via jQuery event listeners in our app, and create ThingFunder instances from our constructor function (unique contracts between each backer and project)! These instances are placed in 'thingFunderInstance' variables for reference. Below, we envoke the payoutToMaker() function of our contract instance, passing in the maker (the current user wallet signed into Metamask retrieved via the eth.accounts method of the Web3 object) and backer (retrieved via a data attribute stored on the DOM... we'll get to this later) addresses.
...
> Clicking the nextInstallment button launches MetaMask, which interfaces with the current blockchain provider via JSON RPC to send money to the project maker via our deployed contract's payoutToMaker() function, asynchronously updating the DOM afterwards.

Now that our app has smart contract functionality, it's just a matter of passing data from our backend database to the smart contract, and updating our ORM with the new information coming from the blockchain. This was tricky because Web3 uses JavaScript and the data that we need to pass to our smart contract is stored in our Rails ActiveRecord ORM. In other words, our data is server-side and our functionality is client-side, and they have to play catch with each other. **In other words, the client-side can't talk to the server-side very easily (they can't play back-and-forth catch with the data).** What I ended up doing was to use controller actions to render data as JSON on page load, then use the JSON to update certain DOM elements' data attributes. Now that the data lives somewhere on the front-end, I'm able to use jQuery to select the values of those data attributes when certain click events are triggered to send that information over to my smart contract.

**3. Making the app asynchronous and instant.**

There's a lot of conditional logic in ThingFunder that changes how the DOM looks. Whether a project has met their funding, whether a project's deadline has come and gone, whether you've donated to a project already, whether you've commented on a project, etc. - all of this affects what information is displayed to the user. Not only that, but this information changes frequently with user interaction. Now that our app has smart contract functionality, we need to make all of this new data, as it changes, show up on our pages in real-time without refreshes (Imagine if you needed to manually refresh the page each time to see whether an email, or payment, or, whatever else, was successfully sent - it would make the internet a lot more annoying and take much longer). 

Enter templating engines and [Handlebars](https://handlebarsjs.com/), a JavaScript library that makes asynchronous DOM changes easier and less messy. When updating the database via AJAX requests, we can already update the DOM by using jQuery and string interpolation of the new data into the DOM's markup. Something like this:
...
My AJAX post request for my 'new_comment' form, when triggered, can then create the comment with the form field params and pass in the returned JSON data into my Handlebars comments template, updating the DOM dynamically.
...
The solution is to not place the event listener on the element itself, but a static parent/container element. This parent element will still be there upon the asynchronous DOM change, and so the event listener will still be intact, and child elements (including the new comment form), will still react to the listener.
