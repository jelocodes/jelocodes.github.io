...
Not to put too much of a damper on things though. For every dude, there are a ton of awesome projects that do get funded and deliver on the goods! However, one or two bad projects are still one or two too many. 
...
A Chrome extension called [Metamask](http://metamask.io) turns Chrome into an Ethereum browser by injecting the Web3.js JavaScript library into the browser, letting websites retrieve data from the blockchain, and letting users manage their cryptocurrency and contracts, via [JSON-RPC](http://en.wikipedia.org/wiki/JSON-RPC). It allows developers to build out the smart contract functionality they want in JavaScript by interacting with that library. In our case, we'll use another helpful library called [EthJS](https://github.com/ethjs) for talking to Ethereum smart contracts via local JavaScript objects. 

The first thing we need to do is check for this global ```web3``` object, which Metamask, if installed, injects into the browser. We need to put a conditional that, if the user does not have Metamask installed, instructs them to download the extension in order to use the app. We can build this out to be prettier, but for now, let's simply pop up a crude alert window. If web3 is detected, we envoke the startApp function and pass the web3 object, and all of its functionality, into it. We trigger this on page load:
...
Afterwards, we need to set up an instance of EthContract with all of the functionality of our specific compiled contract. By passing in the compiled contract's abi as an argument (which we got from Truffle compilation), and instantiating it in a JS constant, we now have access to all of the contract's functions in JS representation, which we can call in our app and pass to Metamask and the blockchain via the injected Web3.js functionality. 
...
Now that our app has smart contract functionality, it was a matter of passing data from our local database to the smart contract, and updating our local database with the new information coming from the blockchain. This was tricky at first because Web3 uses JavaScript and some of the data that we needed to pass it is stored in our Rails ActiveRecord ORM. 
**The client-side can't talk to the server-side very easily (they can't play back-and-forth catch with their data).** What I ended up doing was using controller actions to render data that I needed to pass into Web3 as JSON objects on page load and updating certain DOM elements' data attributes with that JSON information. Then I was able to use jQuery to select those data attributes when certain click events were triggered in order to send that information to the Web3 object.
...
Enter templating engines and Handlebars, a JavaScript library that makes asynchronous DOM changes easier and less messy. When updating the database via an AJAX request, we can update the DOM already by using jQuery and string interpolation of the new data into the DOM's markup in upon the AJAX request succeeding. Something like this:
...
This works well for small requests, but gets messy for bigger ones where we would need to manually build a huge string for this complicated code (and inevitably forget a quotation mark or bracket), so we use templates: pieces of markup with the dynamic data that we want replaced abstracted into general variables. Our templating engine, in this case, Handlebars, knows to replace each of those general variables with the specific data that we feed into it (the 'context' in Handlebars parlance), retrieved from our AJAX request (Handlebars also came with a bunch of [built-in helpers](https://handlebarsjs.com/builtin_helpers.html), and the ability to make custom ones, which was super useful).
...
> In this case, we take the returned JSON data and instead of manually interpolating it into a string, we feed the JSON into our Handlebars template  (in this case, #entry-template) to automatically guarantee the correct based on our provided context (our JSON).

I created multiple Handlebars templates for each section of my markup that I needed dynamically changed every time data was modified on my backend. However, the data I received from my Rails server was not yet in JSON, they were Ruby objects. I needed to modify my controllers to deliver JSON data when requests specified for that format in order for my JavaScript to do anything with it. Luckily, Rails provides the to_json method which takes our object and, well, converts it to JSON.
...
Not bad for a few weeks' work. It's not done by any means, but I learned a lot in a short period of time, and I'm pretty happy with how the project has turned out for now. I'm going to do some more incremental code clean up, then push it up for you guys to try. For now, check out a quick walkthrough of the added features below:
